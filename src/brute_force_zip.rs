...
pub async fn brute_force_zip(input: String) -> Result<String, Box<dyn Error>> {
    ...
    let char_set: Vec<char> = ['a'..='z', '0'..='9']
        .into_iter()
        .flatten()
        .collect();

    let mut password_generator = PasswordGenerator{ char_set: char_set, width: 4, max_width: 6, nth: 0};
        ...
        match archive.by_name_decrypt("secret.txt", pw.as_bytes()) {
            Ok(mut z) => {
                ...
                match z.read_to_string( &mut str){
                    Ok(_) => {
                        Some(String::from(str.strip_suffix("\n").unwrap_or(&str)))
                    },
                    _ => None,
                }
            },
    ...
    let output = Output{secret: secret.unwrap()};
